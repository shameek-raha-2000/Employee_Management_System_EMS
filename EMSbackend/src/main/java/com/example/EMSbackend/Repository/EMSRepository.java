...
//Note - "AND" means check both || "OR" means anyone
	...
	@Query( value = "SELECT * FROM EMPLOYEE_DETAILS s WHERE "
			+ "(:rid IS NULL OR s.E_R_ID  = :rid) AND "
		    + "(:name IS NULL OR s.E_NAME  = :name) AND "
		    + "(:poy IS NULL OR s.E_POY = :poy) AND "
		    + "(:streamName IS NULL OR s.E_STREAM_NAME  = :streamName) AND "
		    + "(:status IS NULL OR s.E_STATUS = :status)",nativeQuery = true)
